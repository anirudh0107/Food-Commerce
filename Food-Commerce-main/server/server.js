...
const checkAdminRole = (req, res, next) => {
  ...
  pool.query('SELECT role FROM anirudh.users WHERE id = $1', [userId])
    .then((result) => {
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      const newUser = await pool.query(
        'INSERT INTO anirudh.users (username, password, email, full_name, image, role) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      ...
      const adminEmail = 'anirudh1502@gmail.com'; // Replace with the admin's email address
      await sendEmail(
        adminEmail,
...
app.get('/pending-registrations', checkAdminRole, async (req, res) => {
  try {
    const pendingUsers = await pool.query('SELECT id, username, email, full_name, image, role FROM anirudh.users WHERE approved = false');
...
app.post('/approve-user', checkAdminRole, async (req, res) => {
  ...
  try {
    const user = await pool.query('SELECT email, full_name FROM anirudh.users WHERE id = $1', [userId]);
    if (user.rows.length === 0) {
      return res.status(404).json({ message: 'User not found' });
    }

    await pool.query('UPDATE anirudh.users SET approved = true WHERE id = $1', [userId]);
...
app.post('/reject-user', checkAdminRole, async (req, res) => {
  ...
  try {
    ...
    await pool.query('DELETE FROM anirudh.users WHERE id = $1', [userId]);
...
app.post('/create-product', uploads.single('photo'), async (req, res) => {
  ...
  try {
    const newProduct = await pool.query(
      'INSERT INTO anirudh.products (name, price, photo, description, category) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
app.get('/products', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, price, description, photo, category FROM anirudh.products ORDER BY id ASC');
...
app.get('/products', async (req, res) => {
  try {
    const result = await pool.query('SELECT id, name, price, description, photo, category FROM anirudh.products');
    res.json(result.rows);
  } catch (err) {
    console.error('Error fetching products:', err);
    res.status(500).send('Server error');
...
app.get('/categories', async (req, res) => {
  try {
    const result = await pool.query('SELECT DISTINCT category FROM anirudh.products');
...
app.get('/products/category/:category', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM anirudh.products WHERE category = $1', [category]);
    res.json(result.rows);
  } catch (err) {
...
app.get('/products/:id', async (req, res) => {
  ...
  try {
    const query = 'SELECT * FROM anirudh.products WHERE id = $1';
...
app.put('/products/:id', (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      let updateQuery = 'UPDATE anirudh.products SET name = $1, price = $2, description = $3, category = $4';
...
app.delete('/products/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query('DELETE FROM anirudh.products WHERE id = $1', [id]);
...
app.post('/add-to-cart', async (req, res) => {
  ...
  try {
    ...
    const existingCartItem = await pool.query(
      'SELECT * FROM anirudh.cart WHERE user_id = $1 AND product_id = $2',
      [userId, productId]
    );
    ...
    if (existingCartItem.rows.length > 0) {
      ...
      const updatedCartItem = await pool.query(
        'UPDATE anirudh.cart SET quantity = $1 WHERE user_id = $2 AND product_id = $3 RETURNING *',
      ...
      const newCartItem = await pool.query(
        'INSERT INTO anirudh.cart (user_id, product_id, quantity) VALUES ($1, $2, $3) RETURNING *',
...
app.get('/cart/:userId', async (req, res) => {
  ...
  try {
    const cartItems = await pool.query(
      `SELECT anirudh.cart.id, anirudh.products.name, anirudh.products.price, anirudh.products.photo, anirudh.cart.quantity, anirudh.cart.added_at 
      FROM anirudh.cart 
      JOIN anirudh.products ON anirudh.cart.product_id = anirudh.products.id 
      WHERE anirudh.cart.user_id = $1`,
      [userId]
    );
...
app.put('/cart/:userId/:itemId', async (req, res) => {
  ...
  try {
    const updatedCartItem = await pool.query(
      'UPDATE anirudh.cart SET quantity = $1 WHERE user_id = $2 AND id = $3 RETURNING *',
...
app.delete('/cart/:userId/:itemId', async (req, res) => {
  ...
  try {
    const deletedCartItem = await pool.query(
      'DELETE FROM anirudh.cart WHERE user_id = $1 AND id = $2 RETURNING *',
      [userId, itemId]
    );
...
app.post('/place-order/:userId', async (req, res) => {
  ...
  try {
    ...
    const cartItemsResult = await pool.query(`
      ...
      FROM anirudh.cart cart
      JOIN anirudh.products products ON cart.product_id = products.id
    ...
    for (let item of cartItems) {
      await pool.query(
        'INSERT INTO anirudh.order_history (order_id, user_id, quantity, status, created_at, product_id) VALUES ($1, $2, $3, $4, $5, $6)',
    ...
    await pool.query('UPDATE anirudh.cart SET delivered = false WHERE user_id = $1', [userId]);

    // Commit transaction
    await pool.query('COMMIT');

    // Get user email and full name
    const userResult = await pool.query('SELECT email, full_name FROM anirudh.users WHERE id = $1', [userId]);
  ...
  try {
      const result = await pool.query(`
          SELECT 
              ...
              p.name,
              p.price
          FROM anirudh.order_history o
          JOIN anirudh.products p ON o.product_id = p.id
...
app.delete('/orders/:userId', async (req, res) => {
  ...
  try {
    ...
    const orderResult = await pool.query('SELECT order_id, product_id, quantity FROM anirudh.order_history WHERE user_id = $1 AND status = $2', 
      [userId, 'pending']);
    ...
    const productDetailsResult = await pool.query('SELECT id, name, price FROM anirudh.products WHERE id = ANY($1)', [productIds]);
    ...
    // Delete orders from anirudh.cart for the user
    await pool.query('DELETE FROM anirudh.cart WHERE user_id = $1', [userId]);

    // Update order history to mark orders as user cancelled
    await pool.query('UPDATE anirudh.order_history SET status = $1 WHERE user_id = $2 AND status = $3',
...
//   const { userId } = req.params;
//   try {
//     await pool.query('UPDATE anirudh.order_history SET status = $1 WHERE user_id = $2', ['user cancelled', userId]);
...
app.put('/orders/:orderId/accept', async (req, res) => {
  ...
  try {
    ...
    const updateResult = await pool.query(
      'UPDATE anirudh.order_history SET status = $1 WHERE order_id = $2 AND status = $3 RETURNING id, user_id, product_id, quantity',
...
app.get('/order-history/:userId', async (req, res) => {
  ...
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) AS total FROM anirudh.order_history WHERE user_id = $1`,
      [userId]
    );
    ...
    const ordersResult = await pool.query(
      `SELECT oh.*, p.photo 
       FROM anirudh.order_history oh
       JOIN anirudh.products p ON oh.product_id = p.id
       WHERE oh.user_id = $1
...
app.post('/submit-rating', async (req, res) => {
  ...
  try {
    ...
    const orderResult = await pool.query('SELECT product_id FROM anirudh.order_history WHERE id = $1 AND user_id = $2', [orderId, userId]);
    ...
    // Insert rating into anirudh.order_history table
    await pool.query('UPDATE anirudh.order_history SET rating = $1 WHERE id = $2', [rating, orderId]);

    // Fetch current rating and rating count from anirudh.products
    const productResult = await pool.query('SELECT rating, rating_count FROM anirudh.products WHERE id = $1', [product_id]);
    ...
    // Update product rating and rating count in the anirudh.products table
    await pool.query('UPDATE anirudh.products SET rating = $1, rating_count = $2 WHERE id = $3', [newRating, newRatingCount, product_id]);
...
app.get('/product-rating/:productId', async (req, res) => {
  ...
  try {
    // Fetch rating and rating count from anirudh.products
    const queryText = `
      SELECT 
        rating, 
        rating_count 
...
//   try {
//     // Fetch rating and rating count from anirudh.order_history based on user_id
//     const result = await pool.query('SELECT rating FROM anirudh.order_history WHERE user_id = $1', [userId]);
//     console.log(result);
//     // return;
...
app.post('/api/contact', async (req, res) => {
  ...
  // return;
  try {
    const query = 'INSERT INTO anirudh.contacts (name, email, food, message) VALUES ($1, $2, $3, $4)';
...
app.get('/api/orders', async (req, res) => {
  ...
  let query = `SELECT * FROM anirudh.order_history WHERE 1=1`;
  ...
  try {
    ...
    const total = await pool.query(`SELECT COUNT(*) FROM anirudh.order_history WHERE 1=1`);
...
//   try {
//     const result = await pool.query('SELECT DISTINCT user_id FROM anirudh.order_history');
//     res.json(result.rows);
//   } catch (err) {
//     console.error(err);
//     res.status(500).send('Server Error');
...
app.get('/api/users', async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM anirudh.users u
      JOIN anirudh.order_history oh ON u.id = oh.user_id
    `);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
...
// Endpoint to fetch messages from anirudh.contact table
app.get('/api/getMessages', async (req, res) => {
  try {
    const client = await pool.connect();
    const result = await client.query('SELECT * FROM anirudh.contacts order by name desc');
